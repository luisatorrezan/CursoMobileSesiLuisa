...
class FirestoreController {
  ...
  void addFavoriteMovie(Map<String,dynamic> movieData) async{
    //carregar imagem diretamente do cachê do aplicativo 
    if(movieData["porterPath"] == null) return; //se filme não tiver poster, não adiciona

    //baixando a imagem do poster
    final imagemUrl = "https://image.tmdb.org/t/p/w500${movieData["porterPath"]}";
    final responseImg = await http.get(Uri.parse(imagemUrl));

    //armazenar a imagem no cache do aplicativo
    final tempDir = await getApplicationCacheDirectory(); //armazer na imagem no aplicativo
    final file = File("${tempDir.path}/${movieData["id"]}.jpg");
    await file.writeAsBytes(responseImg.bodyBytes); //escreve a imagem no arquivo
  }
}